...
pub fn handle_post_drip(ctx: Context<PostDrip>, _params: PostDripParams) -> Result<()> {
    let PostDrip {
        signer,
        global_config,
        pair_config,
...
fn validate_pre_drip_ix_present(ctx: &Context<PostDrip>) -> Result<()> {
    ...
    loop {
        ...
        if ix.program_id.eq(&crate::id()) && current_ix.program_id.eq(&crate::id()) {
            ...
            // TODO: On top of the discriminator, verify that the accounts match what's passed into post-drip
