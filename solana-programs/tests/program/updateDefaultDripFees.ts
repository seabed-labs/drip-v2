...
describe("Program - updateDefaultDripFees", () => {
  ...
  it("updates the default drip fees as super admin", async () => {
    const globalConfigAccountBefore = await program.account.globalConfig.fetch(
      globalConfigPubkey
    );
    ...
    expect({
      ...
      admins: globalConfigAccountBefore.admins.map((admin) => admin.toString()),
      adminPermissions: globalConfigAccountBefore.adminPermissions.map((p) =>
        p.toString()
      ),
      defaultDripFeeBps: globalConfigAccountBefore.defaultDripFeeBps.toString(),
    }).to.deep.equal({
      version: "0",
    ...
    const globalConfigAccountAfter = await program.account.globalConfig.fetch(
      globalConfigPubkey
    );
    ...
    expect({
      ...
      admins: globalConfigAccountAfter.admins.map((admin) => admin.toString()),
      adminPermissions: globalConfigAccountAfter.adminPermissions.map((p) =>
        p.toString()
      ),
      defaultDripFeeBps: globalConfigAccountAfter.defaultDripFeeBps.toString(),
    }).to.deep.equal({
      version: "0",
      superAdmin: superAdminKeypair.publicKey.toBase58(),
      admins: Array<PublicKey>(20)
