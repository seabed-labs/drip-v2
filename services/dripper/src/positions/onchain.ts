...
export class OnChainPositionsFetcher implements IPositionsFetcher {
    ...
    async getPositionsPendingDrip(limit: number = 20): Promise<IPosition[]> {
        const res: IPosition[] = [];
            ...
            filters: [
                ...
                },
                {
                    memcmp: {
                        ...
                        bytes: bs58.encode(Accounts.DripPosition.discriminator)
                    }
                }
            ],
        });
        ...
        while (i < positionKeys.length && res.length < limit) {
            ...
            const dripPosition = await Accounts.DripPosition.fetch(this.connection, positionKey, this.programId)
            if (dripPosition) {
                if (Date.now() > dripPosition.dripActivationTimestamp * BigInt(1000)) {
                    res.push(new Position(dripPosition, await this.getPositionHandler(dripPosition)))
                }
            }
            i += 1
        }
